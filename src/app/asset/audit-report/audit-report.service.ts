...
export class AuditReportService extends UnsubscribeOnDestroyAdapter {
  ...
  private getScheduleActivity = `${this.serverUrl.apiServerAddress}app/auditReport/getList`;
  ...
  getAllList(object){
    console.log(object);
    this.subs.sink = this.httpService.post<AuditReportResultBean>(this.getScheduleActivity,object).subscribe(
      (data) => {
